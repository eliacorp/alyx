...
//retrieve order data
  $scope.retrieveOrder = ()=>{
    ...
    $http({
      ...
      if(!response.data.data.payments[0].data.paid){
        console.log("not paid");
        // $rootScope.completePayment_Paypal();
      }else{
        console.log(response);
        $rootScope.Processed = {value: true, error:false, data:response.data.data};
        $scope.eraseCart($rootScope.Processed.data.cart_id);
...
// erase shopping cart
$scope.eraseCart =(id)=>{
  $http.post('/api/cart/'+id+'/delete')
  .then( function(response){
    console.log(response);
    $rootScope.eraseCookie('cart');
    $rootScope.Cart={};
